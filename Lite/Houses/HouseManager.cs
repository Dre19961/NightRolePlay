...
namespace Lite.Houses
{
    ...
    class HouseManager : Script
    {
        ...
        public static List<HouseType> HouseTypeList = new List<HouseType>
        {
            ...
            new HouseType("Элитный", new Vector3(346.6991, -1013.023, -100.3162), new Vector3(349.5223, -994.5601, -99.7562), 264.0f, "hei_hw1_blimp_interior_v_apart_midspaz_milo_"),
            new HouseType("Премиум", new Vector3(-17.85757, -589.0983, 88.99482), new Vector3(-38.84652, -578.466, 88.58952), 50.8f, "apa_ch2_05e_interior_0_v_mp_stilts_b_milo_"),
        };
        ...
        [RemoteEvent("GoGarageHouse")]
        public static void HouseEnterGarage(Player player)
        {
            House house = Houses.FirstOrDefault(h => h.ID == player.GetData<int>("HOUSEID"));
        ...
        public static void GoHouseMenuInform(Player player, int act)
        {
            ...
            if (house.Locked)
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Двери закрыты! Вас тут не ждали!", 3000);
                return;
            }
            house.SendPlayer(player);
        }
        ...
        private static void callback_selectedcar(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                ...
                case "evac":
                    ...
                    if (VehicleManager.Vehicles[number].Spawn == 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ваше авто находится на Штраф-Стоянке.", 3000);
                        return;
                    }
                    var pos = new Vector3(1, 0, 0);
                    if (VehicleManager.Vehicles[number].Position == JsonConvert.SerializeObject(pos))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы должны заехать в гараж.", 3000);
                        return;
                    }
                ...
                case "spawnmycar":
                    ...
                    if (!check)
                    {
                        if (number != null)
                        {
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Ваша машина будет доставлена в течении 2-х секунд", 3000);
                            NAPI.Task.Run(() =>
                            {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomLeft, "Эта машина не стоит в гараже", 3000);
                        return;
                    }
                    return;
                case "back":
