...
using Lite.Houses;
using Lite.Core;
using static Lite.Core.VehicleManager;
using Lite;
...

{
    class ParkManager : Script
    {
        ...
        {
            if (!Lite.VehicleHandlers.VehiclesName.ModelList.ContainsKey(model))
            {
                return model;
            }
            else
            {
                return Lite.VehicleHandlers.VehiclesName.ModelList[model];
            }
        }
        ...
        internal class ParkBuy
        {
            ...
            {
                Position = pos;
                blip = NAPI.Blip.CreateBlip(50, pos + new Vector3(-1, 0, 0), 0.8f, 69, "Парковка", 255, 0, true);
