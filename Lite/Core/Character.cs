    ...
    public class Character : CharacterData
    {
            ...
            try
            {
                NAPI.Task.Run(() =>
                {
                    try
                    {
                        ...
                        if (IsBonused)
                        {
                            Trigger.ClientEvent(player, "UpdateLastBonus", $"Завтра"); //todo lastbonus
                        }
                        else
                        {
                        ...
                        Casino.Roullete.playergoanimtoall(player);
                        Trigger.ClientEvent(player, "casinomoney", Money);
                        ...
                        if(AdminLVL > 0) player.SetSharedData("REDNAME", true); 
                        ...
                        if (house != null)
                        {
                            ...
                            //.PlayerEvent(player, "createCheckpoint", 333, 27, GarageManager.Garages[house.GarageID].Position - new Vector3(0, 0, 0.2f), 1, NAPI.GlobalDimension, 0, 86, 214);
            ...
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    foreach (DataRow Row in result.Rows)
                    {
                        ...
                        Anick = Convert.ToString(Row["anick"]);
                        ...
                        if (!CaseController.CasesPrizeList.ContainsKey(UUID))
                        {
                            ...
                            MySQL.Query($"INSERT INTO `cases` (`UUID`, `Prize`) VALUES({UUID},'{JsonConvert.SerializeObject(CaseController.CasesPrizeList[UUID])}')");
                        }
                        ...
                        if (Row["pos"].ToString().Contains("NaN"))
                        {
                            ...
                            if (LVL <= 1) SpawnPos = new Vector3(-818.772, -1339.8074, 7.5); // На спавне новичков
                            else SpawnPos = new Vector3(-818.772, -1339.8074, 7.5); //На спавне новичков у мэрии 
                        }
                    }
        ...
        public async Task<bool> Save(Player player)
        {
            try
            {
                ...
                await MySQL.QueryAsync($"UPDATE `characters` SET `pos`='{pos}',`gender`={Gender},`health`={Health},`armor`={Armor},`lvl`={LVL},`exp`={EXP}," +
                    $"`money`={Money},`bank`={Bank},`work`={WorkID},`fraction`={FractionID},`fractionlvl`={FractionLVL},`anick`='{Anick}', `familycid`='{FamilyCID}',`familyrank`={FamilyRank},`arrest`={ArrestTime},`fines`={Fines}," +
        ...
        public async Task<int> Create(Player player, string firstName, string lastName)
        {
            try
            {
                ...
                SpawnPos = new Vector3(-818.772, -1339.8074, 7.5);
                ...
                await MySQL.QueryAsync($"INSERT INTO `characters`(`uuid`,`personid`,`firstname`,`anick`,`lastname`,`gender`,`health`,`armor`,`lvl`,`exp`,`money`,`bank`,`work`,`fraction`,`fractionlvl`,`familycid`,`familyrank`,`arrest`,`fines`,`demorgan`,`wanted`," +
                    ...
                    $"VALUES({UUID},'{PersonID}','{FirstName}','{Anick}','{LastName}',{Gender},{Health},{Armor},{LVL},{EXP},{Money},{Bank},{WorkID},{FractionID},{FractionLVL},'{FamilyCID}',{FamilyRank},{ArrestTime},{Fines},{DemorganTime}," +
