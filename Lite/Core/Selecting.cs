            ...
            try
            {
                ...
                switch (index)
                {
                    ...
                    case 4:
                        if (player.IsInVehicle)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете эвакуировать авто, находясь в машине", 3000);
                            return;
                        }
                        if (vehicle.HasData("ACCESS") && vehicle.GetData<string>("ACCESS") == "FRACTION" && vehicle.GetData<int>("FRACTION") != Main.Players[player].FractionID)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете эвакуировать Фракционное Авто", 3000);
                            return;
                        }
                        if (vehicle.NumberPlate == "ADMIN")
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете эвакуировать Админ-Авто", 3000);
                            return;
                        }
                        if (Fractions.Manager.FractionTypes[Main.Players[player].FractionID] != 2)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете эвакуировать авто, т.к. вы не гос служащий", 3000);
                            return;
                        }
                        if (player.Position.DistanceTo(vehicle.Position) > 5)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Машина слишком далеко от Вас", 3000);
                            return;
                        }
                        NAPI.Entity.SetEntityPosition(vehicle, new Vector3(1, 1, 1));
                        NAPI.Entity.SetEntityDimension(vehicle, 1000);
                        string number = vehicle.NumberPlate;
                        VehicleManager.Vehicles[number].Spawn = 1;
                        VehicleManager.Save(number);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы эвакуировали авто.", 3000);
                        MoneySystem.Wallet.Change(player, +250);
                        return;
                    case 5:
                        if (player.IsInVehicle)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете вернуть машину, находясь в ней", 3000);
                            return;
                        }
                        if (vehicle.HasData("ACCESS") && vehicle.GetData<string>("ACCESS") == "FRACTION")
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете вернуть Фракционное Авто", 3000);
                            return;
                        }
                        if (vehicle.NumberPlate == "ADMIN")
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете вернуть Админ-Авто", 3000);
                            return;
                        }
                        if (!vehicle.HasData("Hijacking"))
                        {
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Это авто не находиться в угоне.", 3000);
                            return;
                        }
                        if (Fractions.Manager.FractionTypes[Main.Players[player].FractionID] != 2)
                        {
                        ...
                        vehicle.Delete();
                        var count = 250 * Main.pluscost;
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы получили {count}.", 3000);
                        return;
                    case 6:
                        ...
                        foreach (DataRow Row in resultt.Rows)
                        {
                            ...
                            Core.VehicleManager.VehicleCustomization datatun = Lite.CarMarket.Custom[data.Number];
