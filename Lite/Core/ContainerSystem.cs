...
namespace Lite.Core
{
    class ContainerSystem : Script
    {
        ...
        public static void OnResourceStart()
        {
            try
            {
                Blip blip = NAPI.Blip.CreateBlip(50, new Vector3(1214.411, -2989.783, 0), 0.7f, 6, Main.StringToU16("Аукцион контейнеров"), 255, 0, true, 0, 0);
                ...
                foreach (DataRow Row in result.Rows)
                {
                    ...
                    foreach (var item in items)
                    {
                        for (int i = 0; i < item.Value; i++)
                        {
                            ...
                            data.NameLoots.Add(VehicleHandlers.VehiclesName.GetRealVehicleName(item.Key));
                        }
                    }
                    containers.Add(data);
                }
        ...
        public static void ChangeStateContainers(Player player = null, bool state = true)
        {
            ...
            if (state)
                NAPI.Chat.SendChatMessageToAll("!{yellow} [Weazel News]: !{#ffffff}" + "Открыт аукцион контейнеров");
        }
    ...
    public class Container
    {
        ...
        public List<string> NameLoots = new List<string>();
        ...
        public Container(int id, string name, Vector3 pos, Vector3 rot, int price, bool donate = false, string model = "prop_container_04a", string door_l = "prop_cntrdoor_ld_l", string door_r = "prop_cntrdoor_ld_r", string fence = "prop_fncsec_01b")
        {
            ID = id;
            Name = name;
            Price = price;
            ...
            Door_l = NAPI.Object.CreateObject(NAPI.Util.GetHashKey(door_l), pos + new Vector3(-3.40, 1.3, 1.4), rot, 255, 0);
            Door_R = NAPI.Object.CreateObject(NAPI.Util.GetHashKey(door_r), pos + new Vector3(-3.40, -1.3, 1.4), rot, 255, 0);
            Fence = NAPI.Object.CreateObject(NAPI.Util.GetHashKey(fence), pos + new Vector3(-3.40, -1.25, 0.5), rot, 0, 0);

            shape = NAPI.ColShape.CreateCylinderColShape(pos + new Vector3(-3.9, 0, 0), 1f, 2f, 0);
            marker = NAPI.Marker.CreateMarker(1, pos + new Vector3(-3.9, 0, 0), new Vector3(), new Vector3(), 0.5f, new Color(219, 112, 147, 0), false, 0);
            label = NAPI.TextLabel.CreateTextLabel($"Класс: ~b~{Name} \n~r~Контейнер закрыт", pos + new Vector3(-3.9, 0, 1.6), 10f, 0.2f, 0, new Color(255, 255, 255), true, 0);
        ...
        public void Visible(bool state)
        {
            if (state)
            {
                label.Text = $"Класс: ~b~{Name} \nЦена: ~p~{Price}{(Donate ? "DC" : "$")}";
                marker.Color = new Color(219, 112, 147, 180);
            }
            else
            {
                //label.Text = "Неактивен";
                label.Text = $"Класс: ~b~{Name} \n~r~Контейнер закрыт";
                marker.Color = new Color(219, 112, 147, 0);
            }
            State = state;
        }
