...
namespace Lite.Core
{
    class Rentcar : Script
    {
        ...
        public static void RentCar(Player player, string vname, int minutes = 5)
        {
            if (!Main.Players.ContainsKey(player)) return;
            if (!player.HasData("RENTAREAID")) return;
    ...
    class RentArea
    {
        ...
        public RentArea(int iD, int type, Vector3 position, List<Vector3> spawnPositions, List<Vector3> spawnRotations)
        {
            ...
            _blip = NAPI.Blip.CreateBlip(RentAreaBlipSprites[(RentAreaType)Type], Position, 0.8f, 84, RentAreaBlipNames[(RentAreaType)Type], 255, 0, true, 0, 0);
            _shape = NAPI.ColShape.CreateCylinderColShape(Position, 2, 2, 0);
            _marker = NAPI.Marker.CreateMarker(1, Position + new Vector3(-1, 0, -1), new Vector3(), new Vector3(), 0.5f, new Color(129, 159, 235, 255), false, 0);
            _textLabel = NAPI.TextLabel.CreateTextLabel(RentAreaBlipNames[(RentAreaType)Type], Position + new Vector3(0, 0, 1.25f), 10f, 5f, 4, new Color(255, 255, 255), false, 0);
        ...
        {
            return _vehicles[vname] * minutes / 5;
        }
