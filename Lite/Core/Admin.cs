...
namespace Lite.Core
{
    class Admin : Script
    {
        ...
        [RemoteEvent("reportPanel")]
        private static void reportPanel(Player player, String report)
        {
            try
            {
                ...
                ReportSys.AddReport(player, report);
            }
        ...
        public static void sendRedbucks(Player player, Player target, int amount)
        {
            ...
            Trigger.ClientEvent(target, "SetRedBucksInMenu", Main.Accounts[target].RedBucks);
        ...
        public static void setPlayerAdminGroup(Player player, Player target)
        {
            ...
            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"{Main.Players[player].Anick} Выдал Вам админ. права", 3000);
        ...
        public static void delPlayerAdminGroup(Player player, Player target)
        {
            ...
            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"{Main.Players[player].Anick} забрал у Вас админ. права", 3000);
        ...
        public static void teleportTargetToPlayerWithCar(Player player, Player target)
        {
            ...
            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"Администратор {Main.Players[player].Anick} телепортировал Вас к себе", 3000);
        }
        ...
        public static void OffMutePlayer(Player player, string target, int time, string reason)
        {
            try
            {
                ...
                NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}{Main.Players[player].Anick} выдал мут игроку {target} на {time} минут");
        ...
        public static void mutePlayer(Player player, Player target, int time, string reason)
        {
            ...
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}{Main.Players[player].Anick} выдал мут игроку {target.Name} на {time} минут");
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}Причина: {reason}");
        ...
        public static void unmutePlayer(Player player, Player target)
        {
            ...
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}{Main.Players[player].Anick} снял мут с игрока {target.Name}");
        ...
        public static void banPlayer(Player player, Player target, int time, string reason, bool isSilence)
        {
            ...
            if (Main.Players[target].AdminLVL >= Main.Players[player].AdminLVL)
            {
                Commands.SendToAdmins(3, $"!{{#d35400}}[BAN-DENIED] {Main.Players[player].Anick} ({player.Value}) попытался забанить {target.Name} ({target.Value}), который имеет выше уровень администратора.");
                return;
            }
            ...
            if (!isSilence)
                NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}{Main.Players[player].Anick} заблокировал игрока {target.Name} на {time}{banTimeMsg}");
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}Причина: {reason}");
        ...
        public static void hardbanPlayer(Player player, Player target, int time, string reason)
        {
            ...
            if (Main.Players[target].AdminLVL >= Main.Players[player].AdminLVL)
            {
                Commands.SendToAdmins(3, $"!{{#d35400}}[HARDBAN-DENIED] {Main.Players[player].Anick} ({player.Value}) попытался забанить {target.Name} ({target.Value}), который имеет выше уровень администратора.");
                return;
            }
            DateTime unbanTime = DateTime.Now.AddMinutes(time);
            string banTimeMsg = "м";
            ...
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}{Main.Players[player].Anick} ударил банхаммером игрока {target.Name} на {time}{banTimeMsg}");
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}Причина: {reason}");
        ...
        [RemoteEvent("Ac-Detect")]
        public static void AcDetect(Player player, int count)
        {
            foreach (Player p in NAPI.Pools.GetAllPlayers())
            {
                if (p == null || !Main.Players.ContainsKey(p)) return;
                if (Main.Players[p].AdminLVL >= 1)
                    p.SendChatMessage("!{#ff0b06}" + $"[ANTICHEAT] [#{Main.Players[player].UUID}] {player.Name} ({player.Value}): нажал кнопку Insert Нажмет еще раз = бан. Всего нажатий: {count}");
            }
        }
        [RemoteEvent("Ac-Detect1")]
        public static void AcDetect1(Player player, int count)
        {
            foreach (Player p in NAPI.Pools.GetAllPlayers())
            {
                if (p == null || !Main.Players.ContainsKey(p)) return;
        ...
        {
            DateTime unbanTime = DateTime.Now.AddMinutes(9999999);
            string banTimeMsg = "м";
            string reason = "Угроза сервера";
            int time = 9999999;
            if (time > 60)
            {
                banTimeMsg = "ч";
        ...
        public static void offBanPlayer(Player player, string name, int time, string reason)
        {
            ...
            if (target != null)
            {
                if (Main.Players.ContainsKey(target))
                {
                    if (Main.Players[target].AdminLVL >= Main.Players[player].AdminLVL)
                    {
                        Commands.SendToAdmins(3, $"!{{#d35400}}[OFFBAN-DENIED] {Main.Players[player].Anick} ({player.Value}) попытался забанить {target.Name} ({target.Value}), который имеет выше уровень администратора.");
                        return;
                    }
                    else
                    {
            ...
            else
            {
                ...
                {
                    Commands.SendToAdmins(3, $"!{{#d35400}}[OFFBAN-DENIED] {Main.Players[player].Anick} ({player.Value}) попытался забанить {name} (offline), который имеет выше уровень администратора.");
                    return;
                }
            }
        ...
        public static void offHardBanPlayer(Player player, string name, int time, string reason)
        {
            ...
            if (target != null)
            {
                if (Main.Players.ContainsKey(target))
                {
                    if (Main.Players[target].AdminLVL >= Main.Players[player].AdminLVL)
                    {
                        Commands.SendToAdmins(3, $"!{{#d35400}}[OFFHARDBAN-DENIED] {Main.Players[player].Anick} ({player.Value}) попытался забанить {target.Name} ({target.Value}), который имеет выше уровень администратора.");
                        return;
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (targetadminlvl >= Main.Players[player].AdminLVL)
                {
                    Commands.SendToAdmins(3, $"!{{#d35400}}[OFFHARDBAN-DENIED] {Main.Players[player].Anick} ({player.Value}) попытался забанить {name} (offline), который имеет выше уровень администратора.");
                    return;
                }
            }
            ...
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}{Main.Players[player].Anick} ударил банхаммером игрока {name} на {time}{banTimeMsg}");
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}Причина: {reason}");
        }
        ...
        public static void kickPlayer(Player player, Player target, string reason, bool isSilence)
        {
            ...
            if (Main.Players[target].AdminLVL >= Main.Players[player].AdminLVL)
            {
                Commands.SendToAdmins(3, $"!{{#d35400}}[KICK-DENIED] {Main.Players[player].Anick} ({player.Value}) попытался кикнуть {target.Name} ({target.Value}), который имеет выше уровень администратора.");
                return;
            }
            if (!isSilence)
                NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}{Main.Players[player].Anick} кикнул игрока {target.Name} по причине {reason}");
            else
            {
                foreach (Player p in Main.Players.Keys.ToList())
                {
                    ...
                    if (Main.Players[p].AdminLVL >= 1)
                    {
                        p.SendChatMessage($"!{{#f25c49}}{Main.Players[player].Anick} тихо кикнул игрока {target.Name}");
                    }
                }
            }
        ...
        public static void warnPlayer(Player player, Player target, string reason)
        {
            ...
            if (Main.Players[target].AdminLVL >= Main.Players[player].AdminLVL)
            {
                Commands.SendToAdmins(3, $"!{{#d35400}}[WARN-DENIED] {Main.Players[player].Anick} ({player.Value}) попытался предупредить {target.Name} ({target.Value}), который имеет выше уровень администратора.");
                return;
            }
            ...
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}{Main.Players[player].Anick} выдал предупреждение игроку {target.Name} ({Main.Players[target].Warns}/3)");
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}Причина: {reason}");
        ...
        public static void teleportTargetToPlayer(Player player, Player target, bool withveh = false)
        {
            ...
            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"{Main.Players[player].Anick} телепортировал Вас к себе", 3000);
        }
        ...
        public static void adminSMS(Player player, Player target, string message)
        {
            ...
            target.SendChatMessage($"~y~{Main.Players[player].Anick} ({player.Value}): {message}");
            player.SendChatMessage($"~y~{Main.Players[player].Anick} ({player.Value}): {message}");
        }
        public static void answerReport(Player player, Player target, string message)
        {
            ...
            target.SendChatMessage($"~r~[Помощь] ~y~{Main.Players[player].Anick} ({player.Value}):~w~ {message}");
            ...
            foreach (Player p in Main.Players.Keys.ToList())
            {
                ...
                if (Main.Players[p].AdminLVL >= 1)
                {
                    p.SendChatMessage($"~b~[ANSWER] {Main.Players[player].Anick}({player.Value})->{target.Name}({target.Value}): {message}");
                }
            }
        ...
        public static void adminChat(Player player, string message)
        {
            ...
            foreach (Player p in Main.Players.Keys.ToList())
            {
                ...
                if (Main.Players[p].AdminLVL >= 1)
                {
                    p.SendChatMessage("!{#2b8234}" + $"[Админ-чат] {Main.Players[player].Anick} ({player.Value}): {message}");
                }
            }
        }
        public static void adminGlobal(Player player, string message)
        {
            ...
            NAPI.Chat.SendChatMessageToAll("!{#7D8AFF }" + $"Администратор {Main.Players[player].Anick}: {message}");
        ...
        public static void sendPlayerToDemorgan(Player admin, Player target, int time, string reason)
        {
            ...
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}Администратор {Main.Players[admin].Anick} посадил в деморган игрока {target.Name} на {time}{deTimeMsg}");
            NAPI.Chat.SendChatMessageToAll($"!{{#f25c49}}Причина: {reason}");
        ...
        public static void releasePlayerFromDemorgan(Player admin, Player target)
        {
            ...
            GameLog.Admin($"{Main.Players[admin].Anick}", $"undemorgan", $"{target.Name}");
        }

        #region Demorgan
        public static Vector3 DemorganPosition = new Vector3(1688.237, 2601.2024, 44.44048);
