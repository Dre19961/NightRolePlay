...
namespace Lite.Core
{
    public class SafeMain : Script
    {
        ...
        public void SafeCracker_Init()
        {
            try
            {
                var flowShape = NAPI.ColShape.CreateCylinderColShape(new Vector3(1395.184, 3613.144, 34.9892), 1.5F, 2, 0);
                ...
                var flowBlip = NAPI.Blip.CreateBlip(440, new Vector3(1395.184, 3613.144, 34.9892), 0.8f, 1, Main.StringToU16("Черный рынок"));
                NAPI.Entity.SetEntityDimension(flowBlip, 0);
                flowBlip.ShortRange = true;

                NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~b~Vladimr Nitup"), new Vector3(1395.184, 3613.144, 34.9892) + new Vector3(0, 0, 1.125), 5F, 0.8F, 0, new Color(255, 255, 255));
            ...
            try
            {
                switch (index)
                {
                    ...
                    case 6:
                        ...
                        return;
                    case 7:
                        if (Main.Players[player].Money < 50000)
                        {
                        ...
                        GameLog.Money($"player({Main.Players[player].UUID})", $"server", 50000, $"buyMavr(armor)");
                        ...
                        return;
                    case 8:
                        if (Main.Players[player].Money < 10000)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"У Вас недостаточно средств", 3000);
                            return;
                        }
                        var tryAdds = nInventory.TryAdd(player, new nItem(ItemType.Programmer));
                        if (tryAdds == -1 || tryAdds > 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Недостаточно места в инвентаре", 3000);
                            return;
                        }
                        Wallet.Change(player, -10000);
                        nInventory.Add(player, new nItem(ItemType.Programmer));
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы купили Программатор.", 3000);
                        return;
                }
            }
