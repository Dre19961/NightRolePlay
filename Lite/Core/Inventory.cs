...
namespace Lite.Core
{
    class nInventory : Script
    {
        ...
        public static Dictionary<ItemType, float> ItemsWeight = new Dictionary<ItemType, float>
            ...
            {ItemType.Apteka, 0.01f },
            {ItemType.WheatSeeds, 0.1f },
...
{ ItemType.Pistol, NAPI.Util.GetHashKey("w_pi_pistol") },
            ...
            { ItemType.PistolAmmo, NAPI.Util.GetHashKey("prop_box_ammo01a") },
            { ItemType.RiflesAmmo, NAPI.Util.GetHashKey("prop_box_ammo01a") },
            { ItemType.ShotgunsAmmo, NAPI.Util.GetHashKey("prop_box_ammo01a") },
            { ItemType.SMGAmmo, NAPI.Util.GetHashKey("prop_box_ammo01a") },
            { ItemType.SniperAmmo, NAPI.Util.GetHashKey("prop_box_ammo01a") },
            ...
            { ItemType.WheatSeeds, NAPI.Util.GetHashKey("ch_prop_ch_moneybag_01a") },
        ...
        public static Dictionary<ItemType, Vector3> ItemsPosOffset = new Dictionary<ItemType, Vector3>()
        {
            ...
            { ItemType.WheatSeeds, new Vector3(0, 0, -0.99) },
        ...
        public static Dictionary<ItemType, Vector3> ItemsRotOffset = new Dictionary<ItemType, Vector3>()
        {
            ...
            { ItemType.SMGAmmo, new Vector3(0, 0, 0) },
            ...
            { ItemType.WheatSeeds, new Vector3(0, 0, 0) },
        ...
        public static Dictionary<ItemType, int> ItemsStacks = new Dictionary<ItemType, int>()
        {
            ...
            { ItemType.AssaultRifle, 2 },
            { ItemType.CarbineRifle, 2 },
            ...
            { ItemType.PistolAmmo, 1000 },
            { ItemType.RiflesAmmo, 2000 },
            { ItemType.ShotgunsAmmo, 200 },
            { ItemType.SMGAmmo, 2000 },
            { ItemType.SniperAmmo, 100 },
            ...
            { ItemType.WheatSeeds, 100 }, //100 семян всего в инвентаре (максимум)

            { ItemType.GiveBox, 100 },

            { ItemType.Programmer, 10},
        };
        ...
        { //if items dict does not contains account uuid, then add him
            var result = MySQL.QueryRead($"SELECT * FROM `inventory` WHERE `uuid` = {uuid}");
            Log.Debug("Player вывод");
                ...
                if (nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type))
                {
                    if (item.IsActive)
                    {
                        ...
                        if (item.FastSlots > 0)
                        {
                            ...
                            player.SetData("AttachWeapon", false);
                        }
                    }
                    else
                    {
                        ...
                        if (item.FastSlots > 0)
                        {
                            ...
                            player.SetData("AttachWeapon", true);
                        }
                ...
                switch (item.Type)
                {
                    ...
                    case ItemType.Bong:
                        if (!player.HasData("USE_DRUGS") || DateTime.Now > player.GetData<DateTime>("USE_DRUGS"))
                        {
                            ...
                            player.SetData("UnderBong", DateTime.Now.AddMinutes(5));
                            NAPI.Task.Run(() => {
                                try
                                {
                                    if (player != null)
                    ...
                    case ItemType.RGBBong:
                        if (!player.HasData("USE_DRUGS") || DateTime.Now > player.GetData<DateTime>("USE_DRUGS"))
                        {
                            ...
                            player.SetData("HEAL_TIMER_BONG", DateTime.Now.AddMinutes(5));
                            NAPI.Task.Run(() => {
                                try
                                {
                                    if (player != null)
                            ...
                            else
                            {
                                ...
                                switch (Main.Players[player].FractionID)
                                {
                                    ...
                                    case 1:
                                        ...
                                        Customization.CustomPlayerData[Main.Players[player].UUID].Clothes.Bodyarmor.Variation = 67;
                                        break;
                                    case 2:
                                        Customization.CustomPlayerData[Main.Players[player].UUID].Clothes.Bodyarmor.Texture = 3;
                                        Customization.CustomPlayerData[Main.Players[player].UUID].Clothes.Bodyarmor.Variation = 67;
                                        break;
                                    case 3:
                                    ...
                                    case 7:
                                        Customization.CustomPlayerData[Main.Players[player].UUID].Clothes.Bodyarmor.Texture = 1;
                                        Customization.CustomPlayerData[Main.Players[player].UUID].Clothes.Bodyarmor.Variation = 65;
                                        break;
                                    case 8:
                                        ...
                                        break;
                                    case 9:
                                        Customization.CustomPlayerData[Main.Players[player].UUID].Clothes.Bodyarmor.Texture = 0; //FBI 
                                        Customization.CustomPlayerData[Main.Players[player].UUID].Clothes.Bodyarmor.Variation = 65;
                                        break;
                                    case 10:
                    ...
                    case ItemType.Drugs:
                        if (!player.HasData("USE_DRUGS") || DateTime.Now > player.GetData<DateTime>("USE_DRUGS"))
                        {
                            ...
                            player.SetData("indrugs", DateTime.Now.AddMinutes(3));
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Попробуйте использовать позже", 3000);
