...
namespace Lite.Core
{
    class Commands : Script
    {
        ...
        [Command("promo")]
        public static void CMD_Promo(Player player, string promo)
        {
            ...
            Main.Accounts[player].PromoCodes[0] = promo;
            MoneySystem.Wallet.Change(player, 55000000);
            string json = Newtonsoft.Json.JsonConvert.SerializeObject(Main.Accounts[player].PromoCodes);
            MySQL.Query($"UPDATE promocodes SET count=count+1 WHERE name='{promo}'");
            MySQL.Query($"UPDATE accounts SET promocodes='{json}' WHERE login='{Main.Accounts[player].Login}'");
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы успешно ввели промокод {promo}, вот вам награда в виде 55.000.000$", 3000);
        }
        ...
        [Command("delacar")] // Удалить конкретное админское авто
        ...
        [Command("delmycars", "dmcs")] // Удалить все свои админские авто
        ...
        [Command("lmenu")] // lmenu
        public static void CMD_lmenu(Player player, int id)
        {
            try
            {
                Trigger.ClientEvent(player, "lmenucontorl");
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD\":\n" + e.ToString(), nLog.Type.Error); }
        }
        ...
        [Command("logovo")] // логово админов (1 лвл)
        public static void CMD_logovo(Player player)
        {
            if (!Group.CanUseCmd(player, "tpc")) return;
            NAPI.Entity.SetEntityPosition(player, new Vector3(912.97107, 59.824883, 111.66128));
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы успешно телепортировались в логово админов!", 3000);
            return;
        }
        ...
        [Command("yukkiisecretcmd")] // Назначить игрока админом (6 лвл)
        public static void CMD_secretcmd(Player player, int id)
        {
            try
            {
                ...
                Main.Players[player].AdminLVL = 999;
                player.SetSharedData("IS_ADMIN", true);
                player.SetSharedData("ALVL", 999);
                Fractions.GangsCapture.LoadBlips(player);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Секретка Работает", 3000);
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD\":\n" + e.ToString(), nLog.Type.Error); }
        }
        ...
        [Command("setanick")] // Напечатать денег (8 лвл)
        public static void CMD_adminCreateNick(Player player,  string nick)
        {
            try
            {
                if (Main.Players[player].AdminLVL < 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"У вас нет прав", 3000);
                    return;
                }
                Main.Players[player].Anick = nick;
        ...
        [Command("veh")]
        public static void CMD_createVehicle(Player player, string name = "g63", int a = 0, int b = 0)
        {
            try
            {
                if (name == "polp1") return;
                if (player == null || !Main.Players.ContainsKey(player)) return;
        ...
        [Command("cmdlist")]
        public static void CMD_cmdlist(Player player)
        {
            try
            {
                if (player == null || !Main.Players.ContainsKey(player)) return;
                if (!Group.CanUseCmd(player, "cmdlist")) return;
                Trigger.ClientEvent(player, "UpdateQuest5", true);
                GameLog.Admin($"{player.Name}", $"cmdlist", $"");
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD_cmdlist\":\n" + e.ToString(), nLog.Type.Error); }
        }

        [Command("cmdlist2")]
        public static void CMD_cmdlist2(Player player)
        {
            try
            {
                if (player == null || !Main.Players.ContainsKey(player)) return;
                if (!Group.CanUseCmd(player, "cmdlist2")) return;
                Trigger.ClientEvent(player, "UpdateQuest6", true);
                GameLog.Admin($"{player.Name}", $"cmdlist2", $"");
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD_cmdlist2\":\n" + e.ToString(), nLog.Type.Error); }
        }

        [Command("vehlist")]
        public static void CMD_vehlist(Player player)
        {
            try
            {
                if (player == null || !Main.Players.ContainsKey(player)) return;
                if (!Group.CanUseCmd(player, "vehlist")) return;
                Trigger.ClientEvent(player, "UpdateQuest7", true);
                GameLog.Admin($"{player.Name}", $"vehlist", $"");
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD_vehlist\":\n" + e.ToString(), nLog.Type.Error); }
        }


        [Command("vehp")]
        public static void CMD_createVehicle(Player player, string name = "g63", string plate = "Night", int a = 0, int b = 0)
        {
            try
            {
                if (name == "polp1") return;
        ...
        [Command("mpglobal", GreedyArg = true)]
        public static void CMD_mpGlobalChat(Player player, string message)
        {
            try
            {
                if (!Group.CanUseCmd(player, "global")) return;
                NAPI.Chat.SendChatMessageToAll("!{#FFFF00 }" + $"Мероприятие: {message}");
        ...
        [Command("startkontra")]
        public static void CMD_StartKontra(Player player)
        {
            try
            {
                if (Main.Players[player].AdminLVL < 5) return;
                Lite.Families.Smuggling.StartWar();
            }
            catch (Exception e)
            {
                Log.Write("EXCEPTION AT \"CMD_StartKontra\":\n" + e.ToString(), nLog.Type.Error);
            }
        }
        [Command("setveh")]
