...
namespace Lite.Families
{
    public class Manager : Script
    {
        ...
        public static int CreatePrice = 70000000; //цена, которая требуется, чтобы создать семью
        public static int NeedLVL = 2; //требуемый уровень игрока, для создания семьи
        ...
        public void StartFamilyManager()
        {
            ...
            try
            {
                NAPI.Blip.CreateBlip(675, PositionFamilyCreator, 0.7f, 55, "Семьи", shortRange: true, dimension: 0);
                NAPI.Marker.CreateMarker(2, PositionFamilyCreator + new Vector3(0, 0, 0.5), new Vector3(), new Vector3(), 0.5f, new Color(255, 225, 64), false, 0);
                shape = NAPI.ColShape.CreateCylinderColShape(PositionFamilyCreator, 1.5f, 3, 0);
                shape.OnEntityEnterColShape += (s, ent) =>
                {
                    try
                    {
                        NAPI.Data.SetEntityData(ent, "INTERACTIONCHECK", 5022);
                    }
        ...
        public static void OpenFamilyMenu(Player player)
        {
            try
            {
                ...
                foreach(var pair in Lite.Core.VehicleManager.getAllPlayerVehicles(Main.PlayerNames[ family.Leader]))
                    pairs.Add( new string[] {(Lite.Core.VehicleManager.Vehicles[pair].Model), pair, Convert.ToString( family.Vehicles != null && family.Vehicles.ContainsKey(pair) ? family.Vehicles[pair] : 0 ) });
                ...
                Lite.Trigger.PlayerEvent(player, "openFamilyMenu", json);
            }
            catch (Exception e) 
            { 
        ...
        static void fcSetMoney(Player player)
        {
            try
            {
                ...
                Lite.Trigger.PlayerEvent(player, "openInput", $"Положить деньги в казну семьи ({Family.GetFamilyToCid(player).Money}$)", "Введите сколько вы хотите положить", 30, "setMfamily");
            }
            catch { }
        }
        ...
        static void fcGetMoney(Player player)
        {
            try
            {
                ...
                Lite.Trigger.PlayerEvent(player, "openInput", $"Снять деньги с казны семьи ({Family.GetFamilyToCid(player).Money}$)", "Введите сколько вы хотите снять", 30, "getMfamily");
            }
            catch { }
        }
        ...
        static void fcBuyitem(Player player, int index)
        {
            try
            {
                ...
                Family family = Lite.Families.Family.GetFamilyToCid(player);
        ...
        public static void LoadFamilyMembersToMenu(Player player)
        {
            try
            {
                ...
                Lite.Trigger.PlayerEvent(player, "loadfamilymemberstomenu", json);
            }
            catch (Exception e)
            {
                Log.Write("Loaded Error \n" + e.ToString(), nLog.Type.Error);
        ...
        public static void OpenCreatorFamilyMenu(Player player)
        {
            try
            {
                ...
                string json = Newtonsoft.Json.JsonConvert.SerializeObject(data);
                Lite.Trigger.PlayerEvent(player, "openCreatorFamilyMenu", json);
            }
            catch (Exception e)
            {
                Log.Write(e.ToString(), nLog.Type.Error);
        ...
        public static void LoadListFamilies(Player player)
        {
            try
            {
                ...
                Lite.Trigger.PlayerEvent(player, "loadlistfamilies", "server", json);
            }
            catch (Exception e)
            {
                Log.Write(e.ToString(), nLog.Type.Error);
