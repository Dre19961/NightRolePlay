...
{
    class HeliCrash : Script
    {

        #region Инициализация
        private static nLog RLog = new nLog("HeliCrash");
        ...
        public static bool showblip = false;
        ...
        {
             new Vector3(468.75967, 2900.951, 43.2233),
             new Vector3(-378.3901, 3976.5996, 54.021053),
             new Vector3(2319.17, -2109.3206, 4.519104)
        };
        ...
        private static List<nItem> Heal = new List<nItem>()
        {
            new nItem(ItemType.HealthKit, 3, "123456789"),
        };

        private static List<List<nItem>> _ammunationBoxLoot = new List<List<nItem>>()
        {
            PistolAmmo, RandomAmmo, AssaultAmmo, BullapAmmo, Heal
        };
        //
        #endregion
            ...
            public static void SpawnAnAmmoBox()
        {
            ...
            _zoneMarker = NAPI.Marker.CreateMarker(1, SpawnPositions[_rndcords] - new Vector3(0, 0, 1), new Vector3(), new Vector3(), 20f, new Color(255, 255, 255, 150), false, 0);
            _ammunationBox = NAPI.Object.CreateObject(NAPI.Util.GetHashKey("prop_crashed_heli"), SpawnPositions[_rndcords], new Vector3(0, 0, 0), 255, 0);
            NAPI.Blip.CreateBlip(612, SpawnPositions[_rndcords], 0.6f, 4, Main.StringToU16("Вертолет с Припасами"), 255, 1, true, 0, 0);
            ...
            NAPI.Notification.SendNotificationToAll($"В штат упал вертолет с {AmmoType[_rndAmmoType]}.", false);
            _isStart = true;
        }
        public static void LoadAmmunitionIntoCar(Player player)
        {
            ...
            Notify.Succ(player, "Машина загружена, поторопись отвезти боеприпасы на склад");
            DestroyBox();
        ...
        {
            Notify.Info(player, "Вы начали загрузку, не выходите с машины!");
            _isLoading = true;
            NAPI.Task.Run(() =>
            {
        ...
        public static void CMD_SpawnBox(Player player)
        {
            try
            {
                if (!Core.Group.CanUseCmd(player, "startheli")) return;
