...
namespace Lite.Fractions
{
    class Fbi : Script
    {
        ...
        private static List<Vector3> fbiCheckpoints = new List<Vector3>()
        {
            ...
            new Vector3(138.33, -702.06116, 32.003376), // garage       7
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                {
                    NAPI.Marker.CreateMarker(1, vec - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1f, new Color(129, 159, 235));
                }

                NAPI.TextLabel.CreateTextLabel("~w~Steve Hain", new Vector3(149.1317, -758.3485, 243.152), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                NAPI.TextLabel.CreateTextLabel("~w~Michael Bisping", new Vector3(120.0836, -726.7773, 243.152), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Переодеться"), new Vector3(fbiCheckpoints[0].X, fbiCheckpoints[0].Y, fbiCheckpoints[0].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));
        ...
        #region menu
        public static void OpenFIBClothesMenu(Player player)
        {
            Menu menu = new Menu("FIBclothes", false, false);
            menu.Callback = callback_FIBclothes;

            Menu.Item menuItem = new Menu.Item("header", Menu.MenuItem.Header);
            menuItem.Text = "Форма";
            menu.Add(menuItem);

            menuItem = new Menu.Item("payday", Menu.MenuItem.Button);
            menuItem.Text = "Переодеться";
            menu.Add(menuItem);

            menuItem = new Menu.Item("combat1", Menu.MenuItem.Button);
            menuItem.Text = "Одежда";
            menu.Add(menuItem);

            menuItem = new Menu.Item("close1", Menu.MenuItem.Button);
            menuItem.Text = "Закрыть";
            menu.Add(menuItem);

            menu.Open(player);
        }
        private static void callback_FIBclothes(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "combat1":
                    MenuManager.Close(client);
                    OpenFIBCombatMenu(client);
                    return;
                case "payday":
                    if (!NAPI.Data.GetEntityData(client, "ON_DUTY"))
                    {
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы начали рабочий день", 3000);
                        Manager.setSkin(client, 9, Main.Players[client].FractionLVL);
                        NAPI.Data.SetEntityData(client, "ON_DUTY", true);
                        break;
                    }
                    else
                    {
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы закончили рабочий день", 3000); ;
                        ...
                        NAPI.Data.SetEntityData(client, "ON_DUTY", false);
                        break;
                    }
                case "close1":
                    MenuManager.Close(client);
        ...
        public static void OpenFIBCombatMenu(Player player)
        {
            Menu menu = new Menu("FIBcombat", false, false);
            menu.Callback = callback_FIBcombat;

            Menu.Item menuItem = new Menu.Item("header", Menu.MenuItem.Header);
            menuItem.Text = "Форма";
            menu.Add(menuItem);

            menuItem = new Menu.Item("FBI1", Menu.MenuItem.Button);
            menuItem.Text = "Форма SWATT";
            menu.Add(menuItem);
        ...
        private static void callback_FIBcombat(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            if (item.ID == "back")
            {
            ...
            switch (item.ID)
            {
                case "FBI1":
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы переоделись в боевую форму", 3000);
                    return;
                case "takeoff":
                    Manager.setSkin(player, Main.Players[player].FractionID, Main.Players[player].FractionLVL);
                    player.SetData("IN_CP_MODE", false);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы сняли боевую форму", 3000);
                    return;
            }
        }
        #endregion
