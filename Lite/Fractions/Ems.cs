    ...
    class Ems : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel("~b~Steve Hobs", new Vector3(), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                NAPI.TextLabel.CreateTextLabel("~b~Billy Moore", new Vector3(), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);

                #region cols
                var col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[1], 1, 2, 0); ;
                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[1], 1, 2, 0); // open hospital stock
                ...
                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[3], 1, 2, 0); // start heal course
                ...
                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[5], 1, 2, 0); // roof
                col.SetData("INTERACT", 63);
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~В больницу"), new Vector3(emsCheckpoints[5].X, emsCheckpoints[5].Y, emsCheckpoints[5].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));

                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[6], 1, 2, 0); // to roof
                col.SetData("INTERACT", 63);
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~На крышу"), new Vector3(emsCheckpoints[6].X, emsCheckpoints[6].Y, emsCheckpoints[6].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));

                #endregion

                for (int i = 3; i < emsCheckpoints.Count; i++)
                {
                    Marker marker = NAPI.Marker.CreateMarker(1, emsCheckpoints[i] - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 0.5f, new Color(35, 152, 247, 255));
                }
            }
        ...
        {
            new Vector3(331.904, -1405.6711, 37.911186),     // Появление после смерти   0
            new Vector3(347.12665, -1428.3428, 31.822496),    // Открыть склад            1
            new Vector3(348.73206, -1426.4252, 31.822496),      // Начать рабочий день      2
            new Vector3(359.2163, -1412.525, 31.822496),     // Начать лечение           3
            new Vector3(374.09848, -1411.6754, 32.509407),     // Удалить тату             4
            new Vector3(355.5395, -1409.0018, 76.16303),     // На крышу                 5
            new Vector3(360.2268, -1397.0557, 31.822496),     // С крыши в больницу       6
        };
