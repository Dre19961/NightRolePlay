                ...
                foreach (var pos in GangEndDelivery)
                {
                    ...
                    colShape.OnEntityEnterColShape += (s, e) =>
                    {
                        try
                        {
                            ...
                            NAPI.Data.ResetEntityData(e.Vehicle.GetData<Player>("WHOS_VEH"), "DELIVERY_CAR");
                            e.Vehicle.Delete();
                            Stocks.fracStocks[Main.Players[e].FractionID].Money += 10000;
                            GameLog.Money($"server", $"frac({Main.Players[e].FractionID})", 10000, "dropCar");
                            e.SendChatMessage("Сдача машины: !{#00FF00}10000$ ~w~были отправлены в общак банды. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()}" + ")");
                        }
